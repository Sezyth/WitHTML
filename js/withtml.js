/*---------------------------------
	...
	This work is licensed under the Creative Commons Attribution - Pas d’Utilisation Commerciale 3.0 non transposé License. To view a copy of this license, visit http://creativecommons.org/licenses/by-nc/3.0/
-----------------------------------*/
...
jQuery(document).ready(function($){
	...
	$('.tooltip, .tooltip-top, .tooltip-bottom, .tooltip-right, .tooltip-left').each(function(){
		...
		if($(this).hasClass('tooltip-top')) position = 'top';
		if($(this).hasClass('tooltip-bottom')) position = 'bottom';
		if($(this).hasClass('tooltip-right')) position = 'right';
		if($(this).hasClass('tooltip-left')) position = 'left';
		
		$(this).attr('title','')
		.tipTip({defaultPosition: position, maxWidth: "250px", content: content, delay: 400});
	});
	...
	$('ul.menu').each(function(){
		...
		.find('a:first').append('<i class="icon-sort-down"></i>');
	});
...
// Vérifie le contenu d'un input #champ
function verifinput(champ)
{
	var erreur = false;
	var classe = champ.className;
	var begin = classe.indexOf("[");
	var end = classe.indexOf("]");
	if (begin > -1 && end > -1)
	{
		classe = classe.substring(begin+1,end+1);
		var clarray = classe.substring(0,classe.indexOf("]")).split('|'); // On explose les conditions dans un tableau
		
		for(var i=0;i<clarray.length;i++)
		{
			if(clarray[i].indexOf("length") == 0) // Nbre de caractères min < nbre < max
			{
				var min = parseInt(clarray[i].substring(clarray[i].indexOf(",min:")+5,clarray[i].indexOf(",max:")));
				var max = parseInt(clarray[i].substring(clarray[i].indexOf(",max:")+5));
				if(max==-1 || max==0)	max = 999999999999999;
				if(champ.value.length < min || champ.value.length > max) erreur = true;
			}
			
			if(clarray[i].indexOf("same") == 0) // Comparaison de champs
			{
				if(document.getElementById(clarray[i].substring(clarray[i].indexOf("same:")+5)).value != champ.value) erreur = true;
			}
			
			if(clarray[i].indexOf("regex") == 0) // Les Regex
			{
				if(clarray[i].substring(clarray[i].indexOf("regex:")+6) == 'mail') //email
				{
					var reg = new RegExp('^[a-z0-9]+([_|\.|-]{1}[a-z0-9]+)*@[a-z0-9]+([_|\.|-]{1}[a-z0-9]+)*[\.]{1}[a-z]{2,4}$', 'i');
					if(reg.exec(champ.value)== null) erreur = true;
				}
				if(clarray[i].substring(clarray[i].indexOf("regex:")+6) == 'date') //date fr
				{
					var reg = new RegExp('^[0-3]{0,1}[0-9]/[0-9]{1,2}/[0-9]{4}$', 'i');
					if(reg.exec(champ.value)== null) erreur = true;
				}
			}
		}
		var sptest = $(champ).attr("onerror");
		if(document.getElementById(sptest)) var sp = sptest; //sp est utilisé pour savoir si on a un span/div à afficher/masquer
		verif_display(champ, erreur,sp);
	}
}

function verif_display(champ, erreur,sp)
{
   if(erreur)
	{
      $(champ).addClass('form-error');
	   if (sp)
                document.getElementById(sp).style.display = "inline-block";
	}
   else
	{
      $(champ).removeClass('form-error');
	  if (sp)
